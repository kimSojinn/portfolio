...
const projectData = {
  "admin-ops-automation": {
    ...
    overview:
      "저작권 모니터링 서비스의 관리자 화면에서는 약 80여 개의 선택 항목을 수동으로 관리하고 있었습니다. 항목이 추가되거나 변경될 때마다 반복적인 수정 작업이 필요했고, 관리 포인트가 많아질수록 운영 부담과 반영 오류 가능성도 함께 증가하는 구조였습니다.\n\n이를 데이터 기반 구조로 리팩토링하여 신규 항목이 자동으로 반영되도록 개선했습니다. 운영자는 별도의 화면 수정 없이 최신 항목을 관리할 수 있게 되었고, 반복적으로 발생하던 운영 작업을 크게 줄일 수 있었습니다.",
    challenge:
    ...
    results: [
      "신규 항목 반영 시간을 90% 이상 단축하여 운영 부담 감소",
      "반복적인 수동 수정 작업을 제거하여 관리 효율 향상",
      "항목 변경 시 자동 반영 구조를 구축하여 운영 안정성 강화",
    ],
  ...
  "seomse-reservation-cx": {
    ...
    results: [
      ...
      "학생창업유망팀 U300 본선 진출을 통해 서비스 기획 및 MVP 검증 경험 확보",
      "이미지 리소스를 13MB에서 1.4MB로 최적화하고 Lighthouse 성능 점수를 77점에서 89점으로 개선",
    ],
  ...
  "service-flow-qa": {
    ...
    overview:
      "저작권 모니터링 서비스와 B2B 관리자 페이지를 개발하며 기획서 기준으로 화면 흐름과 요구사항을 함께 검토했습니다. 기능 구현 과정에서 단순히 화면을 개발하는 데 그치지 않고, 실제 사용 환경에서 흐름이 자연스럽게 이어지는지와 예외 조건이 누락되지 않았는지를 함께 확인했습니다.\n\n또한 운영 과정에서 발생한 추가 요구사항을 기능 개선과 화면 개선으로 반영하며 기획·개발·운영 간 커뮤니케이션 과정에도 참여했습니다. 다국어 환경과 관리자 화면을 검토하며 번역 누락 여부를 확인하고, 권한별 메뉴 노출 조건과 필수 입력값 검증 등 실제 운영 과정에서 발생할 수 있는 예외 상황을 점검했습니다.",
    challenge:
    ...
    results: [
      ...
      "다국어 번역 누락, 권한별 메뉴 노출, 필수 입력값 검증 등 운영 과정의 예외 케이스 점검",
      "클라이언트 요청사항을 기능 단위로 구체화하여 개발 범위와 우선순위 명확화",
    ],
